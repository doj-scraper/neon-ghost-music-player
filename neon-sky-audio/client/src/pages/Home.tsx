...
const STOCK_ARTWORK = "/artwork/stock/default-album.svg";
const BOOT_STEPS = [
  "Core shell online",
  "Universal file intake ready",
  "Playback graph synchronized",
  "Visualizer matrix calibrated",
  "Master bus and limiter armed",
  `Neon Sky Audio ${APP_VERSION} ready`,
] as const;
...
export default function Home() {
  ...
  const [backgroundChoice, setBackgroundChoice] =
    useState<BackgroundType>("stars");
  const [backgroundColor, setBackgroundColor] = useState("#d4af37");
  const [backgroundIndex, setBackgroundIndex] = useState(0);
  const [compactMode, setCompactMode] = useState(false);
  const [compactVariant, setCompactVariant] = useState<
    "compact" | "subcompact" | "artwork"
  >("compact");
  const [subcompactDragActive, setSubcompactDragActive] = useState(false);
  ...
  useEffect(() => {
    const handleKey = (event: KeyboardEvent) => {
      ...
      if (event.code === "ArrowRight") {
        ...
        audioEl.currentTime = clampValue(
  ...
  useEffect(() => {
    setSelectedIds(
      prev =>
        new Set(
          Array.from(prev).filter(id => playlist.some(item => item.id === id))
        )
    );
  }, [playlist]);

  const backgroundOptions = useMemo<
    Array<{ id: BackgroundType; label: string }>
  >(
    () => [
  ...
  const backgroundType = cycleBackgrounds
    ? (backgroundOptions[backgroundIndex % backgroundOptions.length]?.id ??
      "stars")
    : backgroundChoice;
  ...
  const handleVizPrev = () =>
    setVizMode(vizModes[(vizIndex - 1 + vizModes.length) % vizModes.length]);
  const handleVizNext = () =>
    setVizMode(vizModes[(vizIndex + 1) % vizModes.length]);
  ...
  const filteredPlaylist = useMemo(() => {
    ...
    return playlist.filter(item => {
      const haystack =
        `${item.title} ${item.artist} ${item.album} ${item.extension}`.toLowerCase();
      return haystack.includes(query);
    });
  }, [playlist, queueSearch]);

  const totalDuration = useMemo(
    () => playlist.reduce((sum, item) => sum + (item.duration || 0), 0),
    [playlist]
  );
  ...
  const allSelected =
    playlist.length > 0 && selectedIds.size === playlist.length;

  const toggleSelectAll = () => {
    setSelectedIds(
      allSelected ? new Set() : new Set(playlist.map(item => item.id))
    );
  };
  ...
  const onSubcompactDrop = (event: React.DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    setSubcompactDragActive(false);
    const files = Array.from(event.dataTransfer.files ?? []);
    if (files.length === 0) return;
    void addFilesToPlaylist(files, "append");
  };
  ...
  useEffect(() => {
    ...
    const selectedIndex = backgroundOptions.findIndex(
      option => option.id === backgroundChoice
    );
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-black text-white font-mono selection:bg-[#D4AF37] selection:text-black relative overflow-hidden">
      ...
      <ThreeBackground
        ...
        multiColorPulse={shouldMultiPulse}
      />
      ...
      {phase === "player" && (
        <div className="fixed bottom-4 left-4 z-[65] flex flex-col items-start gap-2">
          <button
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center justify-between gap-3">
                  ...
                  <select
                    ...
                    onChange={e =>
                      setBackgroundChoice(e.target.value as BackgroundType)
                    }
      ...
      {phase === "boot" && (
        <div className="fixed inset-0 bg-black flex items-center justify-center z-50 overflow-hidden px-4">
          <div className="absolute inset-0 bg-gradient-to-br from-[#090912] via-[#120b22] to-[#101a3a]" />
          <div className="relative flex flex-col items-center animate-scale-in w-full max-w-2xl">
            <div className="w-28 h-28 sm:w-36 sm:h-36 mb-8 relative">
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-fuchsia-500/20 via-violet-400/15 to-blue-500/25 animate-pulse" />
              <div className="absolute inset-0 border-4 border-violet-300/30 rounded-full animate-ping opacity-20" />
              <div className="absolute inset-0 border-t-4 border-r-4 border-fuchsia-300 rounded-full animate-spin" />
              <div className="absolute inset-2 border-b-4 border-l-4 border-blue-300/50 rounded-full animate-spin-slow" />
              <Disc className="absolute inset-0 m-auto text-white" size={44} />
            </div>
            <h1 className="text-5xl sm:text-7xl font-black tracking-tighter text-white text-center">
              NEON SKY
            </h1>
            <p className="text-white/75 tracking-[0.25em] text-[11px] sm:text-xs mt-3 uppercase text-center">
              Audio Player Boot Sequence {APP_VERSION}
            </p>
            <div className="mt-8 w-full border border-white/10 bg-black/35 backdrop-blur-xl p-5 sm:p-7 space-y-3">
              {BOOT_STEPS.map((step, i) => (
                ...
                >
                  <div className="flex items-center justify-between text-[10px] sm:text-xs uppercase tracking-[0.2em] text-white/80">
                    <span>{step}</span>
                    <span className="text-violet-300">loaded</span>
                  </div>
                  <div className="h-1.5 bg-white/10">
                    <div
                      className="h-full bg-gradient-to-r from-fuchsia-400 via-violet-400 to-blue-400 animate-pulse"
                      style={{ width: `${55 + i * 8}%` }}
                    />
                  </div>
                </div>
            ...
            <button
              onClick={handleInit}
              className="mt-6 w-full py-4 border border-violet-300/60 text-violet-100 font-bold hover:bg-violet-300 hover:text-black transition-all uppercase tracking-[0.25em] text-sm active:scale-[0.98] rounded-none"
            >
              Initialize Player
            </button>
          </div>
      ...
      {phase === "player" && (
        <>
          {isMobile && compactMode && (
            <div
              className={cx(
                "fixed bottom-4 left-3 right-3 z-40 shadow-2xl",
                compactVariant === "artwork"
                  ? "right-auto"
                  : "glass-card rounded-none p-3"
              )}
              onDragOver={
                compactVariant === "subcompact"
                  ? e => {
                      e.preventDefault();
                      setSubcompactDragActive(true);
                    }
                  : undefined
              }
              onDragLeave={
              ...
              onDrop={
                compactVariant === "subcompact" ? onSubcompactDrop : undefined
              }
            >
              {compactVariant !== "artwork" && (
                <div className="mb-2 flex items-center justify-between text-[9px] uppercase tracking-[0.2em] text-white/60">
                  <span>
                    {compactVariant === "subcompact" ? "Subcompact" : "Compact"}
                  </span>
                  <button
                    onClick={() =>
                      setCompactVariant(prev =>
                        prev === "compact"
                          ? "subcompact"
                          : prev === "subcompact"
                            ? "artwork"
                    ...
                    className="h-6 px-2 border border-white/15 bg-white/5 hover:bg-white/10"
                  >
                    Mode
                  </button>
                </div>
              ...
              {compactVariant === "artwork" ? (
                <div className="group relative h-28 w-28 overflow-hidden rounded-2xl border border-white/20 bg-black/50">
                  <img
                    src={track.artwork || STOCK_ARTWORK}
                    alt="Album artwork"
                    className="h-full w-full object-cover"
                  />
                  <div className="absolute inset-0 bg-black/25 opacity-0 transition-opacity group-hover:opacity-100 group-active:opacity-100" />
                  <div className="absolute inset-0 flex items-center justify-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 group-active:opacity-100">
                    <button
                      onClick={prevTrack}
                      className="h-7 w-7 rounded-full bg-black/65 text-white"
                    >
                      <SkipBack size={13} className="mx-auto" />
                    </button>
                    <button
                      onClick={() => void togglePlay()}
                      className="h-9 w-9 rounded-full bg-white/80 text-black"
                    >
                      {isPlaying ? (
                    ...
                    <button
                      ...
                      className="h-7 w-7 rounded-full bg-black/65 text-white"
                    >
                      <SkipForward size={13} className="mx-auto" />
                    </button>
                  </div>
                  <button
                    onClick={() => setCompactVariant("compact")}
                    className="absolute -top-2 -right-2 h-6 w-6 rounded-full border border-white/20 bg-black/70 text-white"
                    aria-label="Expand compact player"
                  >
                ...
                <>
                  ...
                  {compactVariant === "subcompact" && (
                    <div
                      className={cx(
                        "text-center text-[10px] uppercase tracking-[0.2em] py-2 border",
                        subcompactDragActive
                          ? "border-fuchsia-300/80 text-fuchsia-100 bg-fuchsia-400/10"
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => void togglePlay()}
                        className="h-12 w-12 border border-fuchsia-400/60 bg-fuchsia-400/20 text-fuchsia-100 hover:bg-fuchsia-400/40 transition-all active:scale-95"
                        aria-label={isPlaying ? "Pause" : "Play"}
                      >
                        {isPlaying ? (
                          <Pause size={18} className="mx-auto" />
                        ) : (
              ...
              <div className="w-full max-w-[360px] sm:max-w-md glass-card rounded-none p-6 sm:p-10 shadow-2xl flex flex-col items-center gap-5 sm:gap-6 animate-scale-in">
                ...
                <div className="w-full flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${isPlaying ? "bg-green-400 animate-pulse" : "bg-white/20"}`}
                    />
                ...
                <div className="text-center w-full px-2">
                  <span className="text-[9px] sm:text-[10px] text-cyan-400 tracking-[0.4em] sm:tracking-[0.5em] uppercase font-bold">
                    Now Playing
                  </span>
                  <h2 className="text-base sm:text-lg font-bold truncate mt-1.5 text-white/90">
                    {track.title}
                  </h2>
                  ...
                  {track.album && (
                    <p className="text-[10px] sm:text-[11px] text-white/30 truncate mt-0.5">
                      {track.album}
                    </p>
                  )}
                </div>
                  ...
                  >
                    ...
                    <svg className="absolute inset-0" viewBox="0 0 120 120">
                      <defs>
                        ...
                        >
                          ...
                          <stop
                            offset="100%"
                            stopColor="rgba(188,19,254,0.35)"
                          />
                        </linearGradient>
                        <linearGradient
                          id="scrub-progress"
                        ...
                        >
                          ...
                          <stop
                            offset="100%"
                            stopColor="rgba(188,19,254,0.95)"
                          />
                        </linearGradient>
                        <radialGradient
                          id="scrub-handle"
                    ...
                    >
                      {isPlaying ? (
                        ...
                        <Play fill="black" className="ml-1" size={26} />
                      )}
                    </button>
                  </div>
              ...
              <div className="w-full max-w-md glass-card rounded-none shadow-2xl overflow-hidden animate-slide-up">
                <div className="px-6 py-5 flex items-center justify-between border-b border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs font-bold tracking-[0.3em] uppercase text-white/80">
                        Queue
                      </div>
                      <div className="text-[9px] text-white/40 tracking-[0.15em]">
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-1.5">
                              ...
                              <button
                                onClick={() => {
                                  setQueueOpen(false);
                                  void loadTrackAt(playlistIndex, {
                                    autoplay: true,
                                  });
                                }}
                              ...
                              <label className="w-8 h-8 rounded-xl btn-glass flex items-center justify-center cursor-pointer active:scale-90">
                                ...
                                <input
                                  ...
                                  onChange={event => {
                                    const file = event.target.files?.[0];
                                    if (file)
                                      updatePlaylistArtwork(it.id, file);
                                    event.target.value = "";
                                  }}
                                />
                              </label>
          ...
          >
            ...
            <header className="px-4 sm:px-8 py-4 sm:py-5 flex justify-between items-center border-b border-white/5 bg-black/50 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-black tracking-[0.1em] sm:tracking-[0.15em] uppercase">
                    NEON_SKY.ENGINE
                  </h2>
                ...
                <div className="md:col-span-1 lg:col-span-4 flex flex-col gap-5 sm:gap-8">
                  ...
                  <div className="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <label className="text-[10px] text-cyan-400 uppercase tracking-[0.3em] font-bold">
                        Metering
                      </label>
                    </div>
                    <div className="grid grid-cols-2 gap-3 text-[11px] sm:text-xs text-white/70">
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        ...
                        <span className="text-cyan-200 font-semibold font-mono">
                          {formatDb(toDb(meter.peak))} dB
                        </span>
                      </div>
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        <span className="text-white/30 uppercase tracking-[0.15em] text-[9px]">
                      ...
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        ...
                        <span className="text-cyan-200 font-semibold font-mono">
                          {formatDb(meter.lufsMomentary)} LU
                        </span>
                      </div>
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        <span className="text-white/30 uppercase tracking-[0.15em] text-[9px]">
                      ...
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        ...
                        <span className="text-cyan-200 font-semibold font-mono">
                          {formatDb(meter.lufsIntegrated)} LU
                        </span>
                      </div>
                      <div className="flex flex-col gap-1 p-2 rounded-xl bg-white/5">
                        <span className="text-white/30 uppercase tracking-[0.15em] text-[9px]">
                    ...
                    <div className="h-2.5 rounded-full bg-white/10 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${clampValue((meter.correlation + 1) * 50, 0, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  {isDev && (
                    <div className="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          className={cx(
                            ...
                            showFps
                              ? "border-cyan-500/60 text-cyan-200 bg-cyan-500/10 neon-glow-sm"
                        ...
                        <button
                          onClick={() =>
                            setDisableVisualizer(!disableVisualizer)
                          }
                          className={cx(
                            "py-2 rounded-xl border text-[9px] tracking-[0.25em] uppercase transition-all active:scale-95",
                            disableVisualizer
                      ...
                      <div className="text-[9px] text-white/40 uppercase tracking-[0.2em]">
                        Playback:{" "}
                        <span className="text-white/70">{playbackState}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        className="flex-1"
                      />
                      <span className="text-[10px] text-white/70 w-14 text-right font-mono">
                        {targetLufs} LUFS
                      </span>
                    </div>
                    <button
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-6 sm:px-10">
                        {eqBands.map(band => (
                          <div
                            ...
                            style={{
                              transform: `translateY(${-eq[band] * 1.6}px)`,
                            }}
                          />
                        ))}
                      </div>
                      ...
                      {eqBands.map(band => (
                        ...
                        >
                          <div className="flex flex-col sm:flex-row justify-between items-center mb-3 gap-1">
                            <span className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] sm:tracking-[0.3em]">
                              {band}
                            </span>
                          ...
                          <input
                            ...
                            value={eq[band]}
                            onChange={e =>
                              applyBandGain(band, parseInt(e.target.value, 10))
                            }
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                      ...
                      <div className="space-y-3 text-[11px] text-white/60 uppercase tracking-[0.15em]">
                        ...
                        <input
                          ...
                          onChange={e =>
                            setCompressor(prev => ({
                              ...prev,
                              attack: parseFloat(e.target.value) / 1000,
                            }))
                          }
                          className="w-full"
                        />
                        ...
                        <input
                          ...
                          onChange={e =>
                            setCompressor(prev => ({
                              ...prev,
                              release: parseFloat(e.target.value) / 1000,
                            }))
                          }
                          className="w-full"
                        />
                    ...
                    <div className="glass-card rounded-2xl p-5 sm:p-6 space-y-4">
                      ...
                      <div className="space-y-3 text-[11px] text-white/60 uppercase tracking-[0.15em]">
                        <label className="flex items-center justify-between gap-3">
                          <span>Drive</span>
                          <span className="text-cyan-200 font-mono">
                            {Math.round(saturation.drive * 100)}%
                          </span>
                        </label>
                        <input
                          type="range"
                    ...
                    <div className="glass-card rounded-2xl p-5 sm:p-6 space-y-4">
                      ...
                      <div className="space-y-3 text-[11px] text-white/60 uppercase tracking-[0.15em]">
                        ...
                        <button
                          onClick={() =>
                            setStereo(prev => ({ ...prev, mono: !prev.mono }))
                          }
                          className={cx(
                            "w-full py-2.5 rounded-xl border text-[9px] tracking-[0.2em] uppercase transition-all active:scale-95",
                            stereo.mono
                      ...
                      <div className="space-y-3 text-[11px] text-white/60 uppercase tracking-[0.15em]">
                        ...
                        <input
                          ...
                          onChange={e =>
                            setOutput(prev => ({
                              ...prev,
                              trim: parseFloat(e.target.value),
                            }))
                          }
                          className="w-full"
                        />
            ...
            <footer className="px-4 sm:px-8 py-3 sm:py-4 bg-black/80 backdrop-blur-xl border-t border-white/5 flex flex-col sm:flex-row gap-2 sm:gap-4 justify-between items-center text-[9px] sm:text-[10px] text-white/30 uppercase tracking-[0.15em] sm:tracking-[0.25em] font-bold">
              ...
              <div className="flex items-center gap-3 sm:gap-4 flex-wrap justify-center">
                <span className="text-zinc-600 italic hidden sm:inline">
                  "Sound is survival"
                </span>
                <span className="text-white/20">|</span>
                <span>Core v4.5</span>
